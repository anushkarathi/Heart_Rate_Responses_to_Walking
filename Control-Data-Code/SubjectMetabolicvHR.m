...
MetabolicRate=dir([path_directory '/*.mat']); %importing all of the collected data from in-lab part of experiment
MetabolicRate=struct2cell(MetabolicRate);
...
mrtreadmill= regexp(mr, '.*tread.*', 'match'); 
mrtreadmill=mrtreadmill(~cellfun('isempty', mrtreadmill)); 

mroverground= regexp(mr, '.*ovg.*', 'match'); 
mroverground= mroverground(~cellfun('isempty', mroverground)); 
...
%average of the treadmill data and std 
 nameavgmrt=char(mrtreadmill{1}); 
 TMR=load(nameavgmrt, '-mat');
 TMR=struct2array(TMR); 

 %average of the overground data and std 
 nameavgmro=char(mroverground{1}); 
 OMR=load(nameavgmro, '-mat'); 
 OMR=struct2array(OMR); 

 %plots the average HR during treadmill and overground trials as compared to the metabolic rate of subject during trials 
figure 
errorbar(TMR,avgtreadmill, stdevtreadmill, 'kx', 'Linewidth', 3); 
hold on
errorbar(OMR,avgoverground, stdevoverground, 'rx', 'Linewidth', 3); 
xlabel('Metabolic Rate');
ylabel('Average HR');
legend('Treadmill', 'Overground');
title('Metabolic Rate vs HR of Subject');
